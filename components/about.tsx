...
export function About() {
  ...
  const stats = [
    ...
    { number: "1M+", label: "API Calls Handled" },
  ]
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-20">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`text-center p-4 sm:p-6 rounded-xl glass-effect-strong neon-border transition-all duration-1000 transform hover:scale-105 hover:shadow-2xl pulse-glow ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              }`}
          ...
          >
            ...
            <div className="space-y-4 sm:space-y-6 text-muted-foreground leading-relaxed">
              <p className="text-base sm:text-lg fade-in-up" style={{ animationDelay: "0.5s" }}>
                ...
                <code className="glass-effect-strong text-primary px-2 sm:px-3 py-1 rounded font-mono text-xs sm:text-sm neon-border break-all shimmer">
              ...
              <p className="text-sm sm:text-base fade-in-up" style={{ animationDelay: "0.7s" }}>
                With over <strong className="text-primary">5 years</strong> of hands-on experience in enterprise
              ...
              <p className="text-sm sm:text-base fade-in-up" style={{ animationDelay: "0.9s" }}>
                I'm passionate about writing <strong className="text-secondary">clean, maintainable code</strong> and
                implementing industry best practices. Whether it's optimizing database queries, designing resilient
                APIs, or implementing complex business logic, I thrive on solving challenging technical problems.
              </p>
              ...
              >
                <div className="absolute -inset-1 bg-gradient-to-r from-primary/20 to-secondary/20 blur-lg opacity-50"></div>
                <p className="italic relative z-10 text-sm sm:text-base">
                  "Great software is not just about making it work – it's about making it work beautifully, efficiently,
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {highlights.map((highlight, index) => (
                <Card
                  key={index}
                  className={`group hover:shadow-xl transition-all duration-500 transform hover:scale-105 hover:-rotate-1 cursor-pointer border-0 glass-effect-strong neon-border ${highlight.glowColor} bounce-in`}
                  style={{ animationDelay: `${0.5 + index * 0.1}s` }}
                >
                  <CardContent className="p-4 sm:p-6 text-center relative">
                    <div className="absolute -inset-1 bg-gradient-to-br from-primary/5 to-secondary/5 blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-3 sm:mb-4 rounded-full bg-primary/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 relative z-10 shadow-lg ${highlight.glowColor} pulse-glow`}
                    >
        ...
        >
          <div className="max-w-4xl mx-auto">
            <h3 className="text-xl sm:text-2xl font-bold mb-6 sm:mb-8 text-foreground">My Development Philosophy</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
              {[
                ...
                <div
                  key={index}
                  className={`p-4 sm:p-6 rounded-xl glass-effect-strong neon-border hover:shadow-2xl transition-all duration-300 relative group scale-in`}
                  style={{ animationDelay: `${0.7 + index * 0.2}s` }}
                >
                  <div
                    className={`absolute -inset-1 bg-gradient-to-br ${philosophy.color} blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                  ></div>
                  <div className="relative z-10">
                    <div className="text-3xl sm:text-4xl mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300 float-gentle">
                      {philosophy.icon}
                    </div>
